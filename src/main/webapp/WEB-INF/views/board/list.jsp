          ...
          <div class="card shadow-sm">
            ...
            <div class="card-body">
              ...
              이는 부트스트랩을 사용하여 반응형 디자인을 구현하는 데 유용한 기능 중 하나입니다. -->
                ...
                <button class="btn btn-primary" onclick="comment_list(${vo.bno})" type="button" data-bs-toggle="collapse" data-bs-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
				    댓글
				</button>

											<div class="collapse" id="collapseExample">
												<div class="card card-body" id="commentchange">
													<div style="margin: 0 auto; padding-top: 20px; width: 100%;">
														<div id="comment_regist" class="card card-body">
															<span class="right"><input type="button"
																class="btn-fill-s" onclick="comment_regist(${vo.bno})" value="등록"></span>
															<textarea id="comment"
																style="width: 99%; height: 60px; margin-top: 5px; resize: none;"></textarea>
														</div>
														<div class="card card-body" >
														<%--  <input type="hidden" id="pnovalue" value="${vo.bno }"> --%>
															<div
																style="margin: 0 auto; padding-top: 20px; width: 100%;">
...
$('#preview-img').click(function() {
 ...
 function comment_regist(bno) {
	...
	$.ajax({
		...
		data: { bno:bno, comment: $("#comment").val() },
  ...
  function comment_list(bno) {
	$.ajax({
		url: 'board/comment/' + bno,
		//bno를 int로 던지기 위해 board/comment/bno에서 위와 같이 변경
		success: function(data) {
...
/*   $('#commentchange').on('change', function() {
	    var bno = $('#pnovalue').val();
	    $.ajax({
			url: 'board/comment/' + bno,
			//bno를 int로 던지기 위해 board/comment/bno에서 위와 같이 변경
			success: function(data) {
